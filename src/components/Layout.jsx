...
const links = [
  ['/', 'grid-1x2', 'Overview'], ['students', 'people', 'Students'], ['attendance', 'check2-square', 'Attendance'], ['leave', 'envelope-paper', 'Leave'], ['subjects', 'book', 'Subjects'], ['years', 'layers', 'Year master'], ['semesters', 'calendar3', 'Semester master'], ['faculty', 'person-badge', 'Faculty'], ['reports', 'bar-chart-line', 'Reports'], ['settings', 'gear', 'Settings']
];
export default function Layout({ user, onLogout }) {
  const navigate = useNavigate();
  const visibleLinks = links.filter(([to]) => ['admin', 'hod'].includes(user?.role) || !['/years', 'years', 'semesters', 'subjects', 'faculty'].includes(to));
