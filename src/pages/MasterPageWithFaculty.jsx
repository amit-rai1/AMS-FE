...
const configs = {
  ...
  subjects: { title: 'Subject master', singular: 'subject', fields: [['code', 'Subject code'], ['name', 'Subject name'], ['courseName', 'Course'], ['batchName', 'Batch / lab group'], ['facultyName', 'Faculty name']], headers: ['Code', 'Subject', 'Batch', 'Faculty', 'Year', 'Semester', 'Type', 'Status'] },
...
export default function MasterPageWithFaculty({ type }) {
  ...
  const [years, setYears] = useState([]); const [semesters, setSemesters] = useState([]); const [form, setForm] = useState({}); const [editing, setEditing] = useState(null);
  ...
  const loadLookups = async () => { try { const [yearList, semesterList] = await Promise.all([api.get('/years'), api.get('/semesters')]); setYears(yearList.data); setSemesters(semesterList.data); } catch (loadError) { setError(loadError.response?.data?.message || 'Could not load lookup data'); } };
  const load = async () => {
    setLoading(true);
